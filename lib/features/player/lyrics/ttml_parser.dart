...
class Ttml extends Lyric {
  ...
  static Ttml? fromTtmlText(
    ...
    try {
      ...
      final mergedBackgrounds = <TtmlLine>{};
      ...
      lines.removeWhere(mergedBackgrounds.contains);
  ...
  static TtmlLine? _parseParagraph(
    ...
    if (words.isEmpty && text.isNotEmpty) {
      words.add(SyncLyricWord(begin, duration, text));
    }
    if (text.isEmpty &&
        bgSpan == null &&
        translation == null &&
    ...
    if (parsedBegin != null) {
      ...
      if (wordText.isNotEmpty) {
        ...
        words.add(
          SyncLyricWord(
            actualBegin,
            actualEnd == null ? Duration.zero : actualEnd - actualBegin,
            wordText,
          ),
        );
      }
  ...
  static void _fillWordDurations(
    ...
    for (int j = 0; j < words.length; j++) {
      ...
      if (curr.length > Duration.zero) continue;
  ...
  static bool _isOffsetTime(String time) {
    return RegExp(r'^\d+(?:\.\d+)?(?:ms|s|f)$').hasMatch(time.trim());
  }
