...
- 启动与生命周期：启动失败、框架错误、异步错误、前后台切换、网络路线变化。
- Service 连接：端点探测重试与失败、连接成功/断开、HTTP 长时间未完成、超时、TLS/连接/客户端错误分类。
- 实时同步：SSE 建立、重连失败、断流和本次连接处理的事件数量。
- 播放：缓存命中/失效、解析或播放失败、播放器 loading/buffering/ready/completed 状态变化、暂停/音量/倍速/跳转失败、播放完成和播放历史写入失败。
- 业务与本地数据：搜索、下载、歌词、图片解析、缓存维护和本地播放状态持久化失败。

HTTP 请求和播放尝试带有随机短操作编号；编号只用于把同一会话中的开始、完成和失败记录串起来，不包含用户或曲目标识。正常 HTTP 请求只保留完成记录；超过 5 秒仍未结束的请求才额外记录 pending，避免日志被重复请求快速挤满。

日志仍有明确边界：无法可靠捕获原生崩溃、系统强杀、进程卡死和其它 isolate 的未处理错误；播放链路也不会记录音频内容、曲名、搜索词或完整媒体地址。出现这些情况时，诊断包只能提供故障前最后一批本地记录。
